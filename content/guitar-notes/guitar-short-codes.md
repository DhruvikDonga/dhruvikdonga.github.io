...
| `items` | `string` | Optional | Multi-diagram specifications delimited by `;` (`Label | frets:... | mute:... | notes:...`). Must remain on a single continuous line. |
...
### 6. Interactive Scale Animation

Add `animate="true"` to any shortcode to display an interactive player:
* Attaches **Start** and **Stop** badges to sequence endpoints.
...
### 7. Multi-Diagram Selector Tabs (`items=...`)
...
* **Single-Line Attributes:** Never insert line breaks inside a quoted shortcode attribute like `items="..."` or `notes="..."`.
* **Order of Strings:** Always identify strings with numbers `1` (high e) through `6` (low E).
* **Muted Strings:** Declare `mute="5,6"` whenever open bass strings shouldn't ring in chord voicings.
* **Playable Boxes:** Use `box="s1:s2:f1:f2"` to visually box in finger clusters and positional regions.